...
const SponsorLogos = async ({
  ...
  const sortedTiers = [...filteredTiers].sort(
  ...
  const tiersWithSponsors = processedTiers.filter(
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mt-4 px-4">
      ...
      {tiersWithSponsors.map((tier, tierIndex) => {
        const tierStyleProps = getTierStyleProps(
          tier.style,
          getDefaultTierStyle(tier.name),
        );
            ...
            <div className="bg-white p-4 md:p-8 rounded-lg shadow-md">
              <div className={getTierGridClass(tier.id, tier.sponsors.length)}>
                {tier.sponsors.map((sponsor, sponsorIndex) => {
                  const allSponsorsHaveDescriptions = tier.sponsors.every(
                    (s) => !!s.description,
                  );
                  const imageSize =
                    sponsor.width && sponsor.height
                  ...
                  return (
                    <div
                      key={sponsorIndex}
                      className={`flex flex-col items-center justify-center p-4 transition-transform hover:scale-105 duration-300 ${
                        isTopTier ? "relative p-6 rounded-xl  " : ""
                      }`}
                    >
                      {isTopTier && (
                      ...
                      {tiersWithDescriptions.includes(tier.id) &&
                        sponsor.description && (
