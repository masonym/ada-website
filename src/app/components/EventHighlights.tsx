    ...
    <section className="pt-8 pb-16 px-4 bg-gray-100">
      <div className="max-w-[90rem] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map((h, idx) => (
            <div key={idx} className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
              {h.videoId && (
              ...
              <div className="p-6">
                <div className="text-sm text-gray-500 mb-1">{h.sessionTime} - {h.sessionDayDate}</div>
                <div className="font-semibold text-lg mb-2">{h.sessionTitle}</div>
                <button
                  onClick={() => openSpeakersModal(h)}
                  className="inline-block mt-2 text-white px-4 py-2 rounded-md bg-lightBlue-400 hover:bg-blue-500 transition-colors"
                >
                  View Speakers
                </button>
              </div>
