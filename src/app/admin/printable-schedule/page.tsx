...
import { ScheduleDay } from '@/components/SchedulePDF';

// dynamic import to avoid SSR issues with react-pdf
const PrintableSchedule = dynamic(
  () => import('@/components/PrintableSchedule'),
  { 
    ssr: false,
    loading: () => (
...
export default function PrintableSchedulePage() {
  ...
  // get events that have schedules
  const eventsWithSchedules = EVENTS.filter(event => 
    SCHEDULES.some(s => s.id === event.id)
  );
  ...
  const selectedSchedule = selectedEventId
    ? SCHEDULES.find(s => s.id === selectedEventId)?.schedule as ScheduleDay[] | undefined
    : null;

  // fetch speakers when event changes using public API endpoint (includes speakerSlug)
  useEffect(() => {
    if (!selectedEventId) {
