...
export default function PromoCodesPage() {
  ...
  const [editingPromo, setEditingPromo] = useState<SanityPromoCode | null>(
    null,
  );
  ...
  async function handleToggle(promoId: string, currentActive: boolean) {
    try {
      ...
      if (res.ok) {
        setMessage({
          type: "success",
          text: `Promo code ${!currentActive ? "activated" : "deactivated"}`,
        });
        fetchPromoCodes();
      }
    } catch (error) {
  ...
  filteredPromoCodes.forEach((promo) => {
    promo.eligibleEventIds.forEach((eventId) => {
      ...
      if (!groupedPromoCodes[key].find((p) => p._id === promo._id))
        groupedPromoCodes[key].push(promo);
    });
  });
  const sortedEventIds = Object.keys(groupedPromoCodes).sort(
          ...
          <div className="bg-white rounded-xl border p-4 flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold">
                {
                  promoCodes.filter(
                    (p) => p.isActive && !isExpired(p.expirationDate),
                  ).length
                }
              </p>
            </div>
              ...
              >
                <div className="bg-gray-100 px-6 py-4 border-b">
                  ...
                  <p className="text-sm text-gray-600">
                    {groupedPromoCodes[eventId].length} codes
                  </p>
                </div>
                  ...
                  {groupedPromoCodes[eventId].map((promo) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                          ...
                          <span>
                            ...
                            {promo.eligibleTicketTypes.length} types
                          </span>
                        </div>
                      </div>
                      <div className="flex gap-2 ml-4">
                        ...
                        <button
                          onClick={() =>
                            handleToggle(promo._id, promo.isActive)
                          }
                          className={`p-2 rounded-lg ${promo.isActive ? "text-yellow-600 hover:bg-yellow-50" : "text-green-600 hover:bg-green-50"}`}
                        >
                          {promo.isActive ? (
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border rounded-lg font-mono uppercase disabled:bg-gray-100"
                  placeholder="SAVE20"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  value={formDiscount}
                  onChange={(e) =>
                    setFormDiscount(parseInt(e.target.value) || 0)
                  }
              ...
              <div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto border rounded-lg p-3">
                  {EVENTS.filter((e) => e.id > 0).map((event) => (
